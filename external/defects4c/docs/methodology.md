...
| **What build systems are used?** | **Make** (and Autotools: `./configure && make`), **CMake**, and project-specific config (e.g. OpenSSL, nginx). Per-project `build_cmd` entries appear in `data/bug_catalog.json` under `projects_info`, and each bug record also includes its own `build_cmd`. |
